...
def _load_drugcentral_from_disk() -> bool:
    """尝试从磁盘缓存加载 DrugCentral 数据。返回 True 表示成功，False 表示缓存不存在或损坏。"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"[pharma_cache] 下载完成: {url.split('/')[-1]} ({size_mb:.1f} MB)")
                return content
            elif r.status_code == 404:
                print(f"[pharma_cache] 文件不存在: {url}")
                return b''
            else:
...
def _load_drugcentral(force_download=False) -> bool:
    """加载 DrugCentral 数据到内存。使用并发下载（两个文件同时下载）以提升速度。"""
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future_inter = executor.submit(_download_with_retry, session, _DRUGCENTRAL_INTERACTION_URL)
        future_struct = executor.submit(_download_with_retry, session, _DRUGCENTRAL_STRUCTURES_URL)

        try:
            interaction_data = future_inter.result(timeout=300)
        except Exception as e:
            print(f"[pharma_cache] interaction 文件下载超时或失败: {e}")

        try:
            structure_data = future_struct.result(timeout=300)
        except Exception as e:
            print(f"[pharma_cache] structures 文件下载超时或失败: {e}")
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if name_key not in _drugcentral_targets:
                _drugcentral_targets[name_key] = {'targets': [], 'diseases': [], 'actions': []}
...
def _get_pubchem_session() -> requests.Session:
    ...
    if _pubchem_session is None:
        ...
        _pubchem_session.mount('http://', adapter)
        _pubchem_session.mount('https://', adapter)
...
def query_pubchem_targets(compound_name: str, max_targets: int = 10) -> list:
    """通过 PubChem PUG REST 获取化合物的靶点信息"""
    name_key = _normalize_name(compound_name)
    if name_key in _pubchem_cache:
        cached = _pubchem_cache[name_key]
...
def compute_pharma_evidence_score(info: dict) -> tuple:
    ...
    if info['drugcentral_targets']:
        ...
        evidence.append(f"DrugCentral: {n_targets} targets ({tgt})")
        if info['drugcentral_diseases']:
            evidence.append(f"  Disease: {info['drugcentral_diseases'][0]}")
    ...
    if info['pubchem_bioactive']:
        ...
        if info['pubchem_targets']:
            tgt = '; '.join(info['pubchem_targets'][:3])
            evidence.append(f"  Targets: {tgt}")
        if info['pubchem_activities']:
            evidence.append(f"  Activity: {info['pubchem_activities'][0][:60]}")

    if not info['source']:
        evidence.append('No database records')
